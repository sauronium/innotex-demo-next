...
import RDWorkspace from '@/components/modules/rd-workspace';
...
export default function PLMWorkspace({ initialModule = 'master' }: {
    initialModule?: 'master' | 'client' | 'rd';
}) {
    ...
    useEffect(() => {
        ...
        if (selectedWork && initialModule !== 'rd') {
        ...
        if (initialModule === 'rd' && selectedWork) setWorkId(selectedWork);
        if (selected)
    ...
    const rdWorks = scopedWorks.filter(w => w.stage >= 1);
    const work = module === 'rd' ? rdWorks.find(w => w.id === workId) || rdWorks.find(w => w.stage === 1) || rdWorks[0] : works.find(w => w.id === workId) || works[0];
    ...
    return <div className="space-y-6"><header className="flex flex-wrap items-start justify-between gap-4"><div><p className="mb-2 text-xs font-semibold uppercase tracking-[.2em] text-blue-600">Product lifecycle management</p><h1 className="text-3xl font-bold tracking-tight">{module === 'rd' ? 'Research & Development' : 'Every product. One connected journey.'}</h1><p className="mt-2 text-sm text-muted-foreground">From client insight to delivered product, with a shared view of every handover.</p><p role="status" className="mt-2 text-xs text-muted-foreground">{saved}</p></div><Badge variant="outline">PLM / {module === 'rd' ? 'R&D' : module === 'master' ? 'Master PLM' : 'Client PLM'}</Badge></header>
 <nav aria-label="PLM modules" className="flex flex-wrap gap-2 border-b pb-4">{(['master', 'client'] as const).map((m, i) => <Button key={m} variant={module === m ? 'default' : 'outline'} asChild><Link href={m === 'master' ? '/plm/designs' : '/plm/clients'}>{i ? <Users className="mr-2 h-4 w-4"/> : <Layers className="mr-2 h-4 w-4"/>}{i ? 'Client PLM' : 'Master PLM'}</Link></Button>)}<Button asChild variant={module === 'rd' ? 'default' : 'outline'}><Link href="/plm/rd">R&amp;D Workspace</Link></Button></nav>
 <fieldset disabled={!ready} className="min-w-0 space-y-5">
 {module === 'rd' ? <RDWorkspace key={work?.id || 'empty'} data={visibleData} work={work} onSelect={w => setWorkId(w.id)} update={update}/> : module === 'master' ? <>
 ...
 <p className="my-3 text-xs text-muted-foreground">{stageEditable ? "Your role can update this product’s current stage." : "Read-only stage for your role. Select the responsible role to record this handover."}</p><HandoverSummary work={work}/>{(stage === 1 || work.rd) && <Button asChild variant="outline" className="mt-3"><Link href={`/plm/rd?work=${encodeURIComponent(work.id)}`}>Open R&amp;D development details →</Link></Button>}<div className="my-4"/>
